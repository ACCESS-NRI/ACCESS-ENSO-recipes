...
def seasonality_level3(mod_obs_cubes, dt_ls):
    ...
    for label, cube in dict(zip(dt_ls, mod_obs_cubes)).items():
        ...
        c1 = iplt.contourf(cube, cmap='Reds', levels=np.arange(0., 2, 0.1), extend='both')  # Set levels to include <0 and >2
...
def seasonality_level4(mod_obs_cubes, dt_ls):
    ...
    for label, cube in dict(zip(dt_ls, mod_obs_cubes)).items():
        for seas, col in {'NDJ':'red','MAM':'blue'}.items():
            ...
            qplt.plot(cube_plot, color=col, label=f'{label} {seas}', linestyle=lines[label], linewidth=2)
