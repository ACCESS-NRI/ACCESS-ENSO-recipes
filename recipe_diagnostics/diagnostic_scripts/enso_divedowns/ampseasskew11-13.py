"""diagnostic script to plot ENSO metrics 11-13 dive downs"""
...
def seasonality_level3(mod_obs_cubes, dt_ls):
    ...
    for label, cube in dict(zip(dt_ls, mod_obs_cubes)).items():
        ...
        c1 = iplt.contourf(
            cube, cmap="Reds", levels=np.arange(0.0, 2, 0.1), extend="both"
...
def create_legend_seas(dt_ls):
    ...
    legend_elements = [
        ...
        Line2D(
            [0], [0], linestyle="dashdot", color="k", lw=1.5, label=f"Ref: {dt_ls[0]}"
        ),
    ]
...
def compute_enso_metrics(input_pair, dt_ls, var_group, metric):
    ...
    if metric == "11amplitude":
        ...
        fig3 = seasonality_level3(
            [input_pair[0][var_group[1]], input_pair[1][var_group[1]]], dt_ls
        )
        fig4 = seasonality_level4(
            [input_pair[0][var_group[2]], input_pair[1][var_group[2]]], dt_ls
        )
        process = {}
        for seas in ["NDJ", "MAM"]:
            for label, cube in dict(
                zip(dt_ls, [input_pair[0][var_group[3]], input_pair[1][var_group[3]]])
            ).items():
        ...
        for cube in model_obs:
            ...
            asymls.append((cube2.coord("longitude").points, skew(cube2.data, axis=0)))
            # lvl3
    ...
    for metric, var_preproc in metrics.items():  # if empty or try
        ...
        for dataset in model_ds:
            ...
            figs = compute_enso_metrics(
                input_pair, [obs[0]["dataset"], dataset], var_preproc, metric
            )

            dt_files = obs_files + [ds["filename"] for ds in models]
            for i, fig in enumerate(figs):
                prov_record = get_provenance_record(
                    f"ENSO metrics {metric} level {i + 2}", dt_files
                )
