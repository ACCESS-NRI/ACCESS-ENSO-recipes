"""diagnostic script to plot ENSO metrics 14,15 dive downs"""
...
def enso_composite(n34):
    ...
    for enso, years in events.items():
        ...
        durations = [
            threshold_duration(line, 0.5, enso) for yr, line in cube_data.items()
        ]
...
def diversity(ssta_cube, events_dict):
    ...
    ssta_cube = rolling_window_statistics(
        ssta_cube, coordinate="longitude", operator="mean", window_length=5
    )
    ...
    for metric, var_preproc in metrics.items():
        ...
        for dataset in model_ds:
            ...
            prov_record = get_provenance_record(
                f"ENSO metrics {metric} dive down", dt_files
            )

            save_figure(
                f"{dataset}_{metric}_divedown", prov_record, cfg, figure=figs, dpi=300
            )
