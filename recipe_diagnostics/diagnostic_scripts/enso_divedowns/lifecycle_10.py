...
def plot_level2_contour(model_ds, obs_ds, dt_ls):
    ...
    c1 = plot_ssta(
        ax1, obs_ds[0], obs_ds[1], f"{dt_ls[1]}:\nreg(ENSO SSTA, SSTA)", i=None
    )

    # Second subplot: Model
    plot_ssta(
        ax2, model_ds[0], model_ds[1], f"{dt_ls[0]}:\nreg(ENSO SSTA, SSTA)", i=None
    )
...
def create_legend(dt_ls):
    ...
    legend_elements = [
        ...
        Line2D(
            [0], [0], linestyle="dashdot", color="k", lw=1.5, label=f"Ref: {dt_ls[1]}"
        ),
    ]
...
def compute_enso_metrics(input_pair, dt_ls, var_group):
    ...
    fig3 = plot_l3_enso_lifecycle(
        input_pair[1][var_group[0]], input_pair[0][var_group[0]], dt_ls
    )

    fig4 = plot4_enso_contour(
        list(input_pair[1].values()), list(input_pair[0].values()), dt_ls
    )
    ...
    for metric, var_preproc in metrics.items():
        ...
        for dataset in model_ds:
            ...
            figs = compute_enso_metrics(
                input_pair, [dataset, obs[0]["dataset"]], var_preproc
            )
