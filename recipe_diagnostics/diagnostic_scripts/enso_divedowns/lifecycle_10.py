...
def plot_level2_contour(model_ds, obs_ds, dt_ls):
    ...
    c1 = plot_ssta(ax1, obs_ds[0], obs_ds[1], 
                f'{dt_ls[1]}:\nreg(ENSO SSTA, SSTA)', i=None)

    # Second subplot: Model
    c2 = plot_ssta(ax2, model_ds[0], model_ds[1], 
                   f'{dt_ls[0]}:\nreg(ENSO SSTA, SSTA)', i=None)
    ...
    ticks = np.arange(-1.2, 1.3, 0.2)  # Ticks from -1.2 to 1.2 at intervals of 0.2
...
def plot_ssta(ax, area_coordlon, ssta, label, i=None):
    ...
    if label.split()[0] == 'El':
        ...
        axpos.set_xlabel('Longitude') # label.split()[0] El Nino
    else:
...
def enso_composite_plot4(n34_cube, n34_area, label, i, axes):
    ...
    for enso, years in events.items():
        ...
        cplot = plot_ssta(axes[enso], n34_area.coord('longitude').points,
                          means, f'{enso.title()} {label}', i)
...
def plot4_enso_contour(model_ds, obs_ds, dt_ls):
    ...
    cbar = fig.colorbar(c1, ax=[ax2[0], ax2[1]], label='ENSO SSTA °C', orientation='horizontal', 
                        ticks=np.arange(-1.2, 1.3, 0.4), fraction=0.05, pad=0.08)
...
def compute_enso_metrics(input_pair, dt_ls, var_group, metric): 
    ...
    if metric =='10lifecycle':
        ...
        fig3 = plot_l3_enso_lifecycle(input_pair[1][var_group[0]],
                                      input_pair[0][var_group[0]], dt_ls)

        fig4 = plot4_enso_contour(list(input_pair[1].values()), 
                                  list(input_pair[0].values()), dt_ls)
...
def main(cfg):
    ...
    metrics = {'10lifecycle':['tos_lifdur1', 'tos_lifdurdiv2'],
                }
