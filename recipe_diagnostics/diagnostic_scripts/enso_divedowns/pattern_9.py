...
def plot_ensofig(i, cube, label):
    ...
    ax1.coastlines()
    cf2 = iplt.contourf(cube, levels=np.arange(-2,2,0.1),extend='both', cmap='RdBu_r')
...
def plot_maps_pattern(processed_data):
    ...
    cbar = fig.colorbar(cf1, cax=cax, orientation='horizontal', extend='both', ticks=np.arange(-2,2.2,0.5))
...
def plot_maps_pattern4(model_ds, obs_ds, dt_ls):
    ...
    # Add a single colorbar at the bottom
    cax = plt.axes([0.15,0.08,0.7,0.05])
    cbar = fig.colorbar(cf, cax=cax, orientation='horizontal', extend='both', ticks=np.arange(-2,2.2,0.5))
...
def plot_enso_ssta(prep_datasets, line, label):
    ...
    for enso, years in events.items():
        ...
        if enso == 'nina': ## plot separate
            qplt.plot(cube, color='blue', linestyle=line, lw=3, label=f"{label} La Nina")
        else:
            qplt.plot(cube, color='red', linestyle=line, lw=3, label=f"{label} El Nino")

def create_legend(dt_ls):
    """Create a legend for the ENSO composite plot."""
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=8, label='La Niña'),
...
def compute_enso_metrics(input_pair, dt_ls, var_group, metric): 
    ...
    if metric == '09pattern':
        ...
        reg_obs = lin_regress_matrix(input_pair[0][var_group[2]], input_pair[0][var_group[0]])
        processed = dict(zip(dt_ls, [reg_mod, reg_obs]))
        fig2 = plot_maps_pattern(processed)

        fig3 = plot_lvl3_pattern([input_pair[1][var_group[1]], model_nino34],
                                    [input_pair[0][var_group[1]], input_pair[0][var_group[0]]], dt_ls)
        fig4 = plot_maps_pattern4([model_ssta,model_nino34], 
                                    [input_pair[0][var_group[2]], input_pair[0][var_group[0]]], dt_ls)
    ...
    for metric, var_preproc in metrics.items(): #if empty or try
        ...
        for dataset in model_ds:
            ...
            for i, fig in enumerate(figs):
                ...
                save_figure(f'{dataset}_{metric}_level_{i+2}', prov_record, cfg, figure=fig, dpi=300)
