...
def plot_level1(obs_ds, model_ds, title, metric_varls):
    ...
    plt_settings(
        [mod_slope, obs_slope, metric_val], lvl1=True, metric_varls=metric_varls
    )
    plt.title(title)
...
def plot_level2(obs_ds, model_ds, metric_varls, ds_labels):
    ...
    plt.subplot(121)
    plt_lvl2_subplot(
        model_ds[metric_varls[0]], model_ds[metric_varls[1]], ds_labels[1], metric_varls
    )
...
def plt_lvl2_subplot(ts_cube, tauu_cube, dataset_label, metric_varls):
    ...
    slope, intcpt = linreg_1d(
        df.loc[df["tos"] < 0, "tauu"], df.loc[df["tos"] < 0, "tos"]
    )
...
def plt_settings(slopes, lvl1, metric_varls):
    ...
    plt.xticks(
        np.arange(var_set[xvar][0] * -1, var_set[xvar][0] + 1, var_set[xvar][0] / 2)
    )
    plt.ylim(var_set[yvar][0] * -1, var_set[yvar][0])
    plt.yticks(
        np.arange(var_set[yvar][0] * -1, var_set[yvar][0] + 1, var_set[yvar][0] / 2)
    )
    plt.grid(linestyle="--")
...
def obs_extract_overlap(obs_1, obs_2):
    ...
    obs1 = obs_1.extract(
        iris.Constraint(time=lambda t: start_overlap <= t.point <= end_overlap)
    )
    obs2 = obs_2.extract(
        iris.Constraint(time=lambda t: start_overlap <= t.point <= end_overlap)
    )
...
def main(cfg):
    ...
    for metric, var_preproc in metrics.items():  # if empty or try
        ...
        for dataset, mod_ds in model_ds.items():
            ...
            save_figure(
                f"{dataset}_{metric}_lvl2", prov_record, cfg, figure=fig, dpi=300
            )  #
