...
def feedback_nonlin_lvl4(sst_cube, tauu_cube):
    ...
    logger.info(f'{sst_cube.standard_name}, {sst_cube.long_name}, {sst_cube.var_name}, {sst_cube.units}')
    ...
    logger.info(f'ssta_neg: {ssta_neg.shape}, ssta_pos: {ssta_pos.shape}, xbelow0: {xbelow0.shape}, xabove0: {xabove0.shape}')
...
def lin_regress_4(cubeA, cubeBsst):
    ...
    logger.info(f'B_with_intercept shape: {B_with_intercept.shape}, A_data shape: {A_data.shape}')
...
def plot_level_4(obs_ds, model_ds, metric_varls, ds_labels):
    ...
    tau_obcube = rolling_window_statistics(obs_ds[metric_varls[1]], 
                                     coordinate='longitude',operator='mean',window_length=30)
    ...
    rangesx = {'sst':(-20, 20, 2), 'taux':(-0.5, 0.5, 0.05), 'ssh':(-0.3, 0.3, 0.02)}
    ...
    for isub, cbls in enumerate(zip(obsls, modells)):
        for cb in cbls:
            if i == 321:
                ...
                ax1.set_title(f'ref: {ds_labels[0]} \nreg({xvar.upper()}A, {yvar.upper()}A)', loc='left') #label
            else:
                ax2 = plt.subplot(i, sharex=ax1, sharey=ax1)
                ax2.set_title(f'reg({xvar.upper()}A{sub[isub]}, {yvar.upper()}A)', loc='left')
                if i == 322:
                    ax2.set_title(f'{ds_labels[1]} \nreg({xvar.upper()}A, {yvar.upper()}A)', loc='left')

            ## contour plt data
            cf1 = iplt.contourf(cb, coords=['longitude','month_number'], levels=np.arange(*rangesx[xvar]), extend='both', cmap="RdBu_r")
    ...
    tickranges = (rangesx[xvar][0], rangesx[xvar][1]+rangesx[xvar][2], rangesx[xvar][1]*0.5)
    cbar = figure.colorbar(cf1, cax=cax, orientation='horizontal', extend='both', ticks=np.arange(*tickranges))
    unitlabels = {'sst': '°C', 'taux': 'N/m2', 'ssh': 'cm', 1:'1e-3 ', 0:''}
    n = 1 if xvar == 'taux' or yvar == 'taux' else 0
    cbar.set_label(f'regression ({unitlabels[n]} {unitlabels[yvar]}/{unitlabels[xvar]})') ##title (1e-3 cm/N/m2) (°C/cm)
...
def _fix_time_coord(cube, _, _filename): #fix obs data time differs
    """Set time points to same day and hour for monthly data."""
    t_coord = cube.coord("time")
    _unit = t_coord.units
    new_time = [d.replace(day=15, hour=0) for d in _unit.num2date(t_coord.points)]
...
def main(cfg):
    ...
    metrics = {'SST_TAUX': ['sst_east', 'taux_eqp'],
    ...
    for metric, var_preproc in metrics.items(): #if empty or try
        ...
        for dataset, mod_ds in model_ds.items():
            ...
            prov_record = get_provenance_record(f'ENSO metrics {metric} feedback level 4', dt_files)

            obs_ds = {dataset['variable_group']: iris.load_cube(dataset['filename'], callback=_fix_time_coord) for dataset in obs}
            ...
            save_figure(f'{dataset}_{metric}_lvl4', prov_record, cfg, figure=fig, dpi=300)
