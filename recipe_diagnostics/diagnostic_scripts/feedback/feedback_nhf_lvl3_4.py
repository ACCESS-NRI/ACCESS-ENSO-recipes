...
def plot_level3(obs_ds, model_ds, metric_varls, ds_labels, title):  # edit for nhf
    ...
    cnts, neg, pos = feedback_nonlin(
        model_ds[metric_varls[0]], model_ds[metric_varls[1]]
    )
...
def plot_level_4(obs_ds, model_ds, metric_varls, ds_labels):
    ...
    modells = feedback_nonlin(
        model_ds[metric_varls[0]], model_ds[metric_varls[1]], level4=True
    )
    ...
    for isub, cbls in enumerate(zip(obsls, modells)):
        for cb in cbls:
            if i == 321:
                ...
                ax2.set_title(
                    f"reg({xvar.upper()}A{sub[isub]}, {yvar.upper()}A)", loc="left"
                )
                if i == 322:
                    ax2.set_title(
                        f"{ds_labels[1]} \nreg({xvar.upper()}A, {yvar.upper()}A)",
...
def main(cfg):
    ...
    obs_ds = {
        dataset["variable_group"]: iris.load_cube(
            dataset["filename"], callback=_fix_time_coord
        )
        for dataset in obs
    }
    ...
    for dataset, mod_ds in model_ds.items():
        ...
        prov_record = get_provenance_record(
            f"ENSO metrics {metric} feedback level 3", dt_files
        )
        ...
        prov_record = get_provenance_record(
            f"ENSO metrics {metric} feedback level 4", dt_files
        )
        fig = plot_level_4(obs_ds, model, SST_NHF, ds_labels)
        save_figure(
            f"{dataset}_{metric}_lvl4", prov_record, cfg, figure=fig, dpi=300
        )
