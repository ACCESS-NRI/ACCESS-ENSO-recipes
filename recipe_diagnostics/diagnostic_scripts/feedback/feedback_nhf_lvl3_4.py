...
def plot_level3(obs_ds, model_ds, metric_varls, ds_labels, title): #edit for nhf
    ...
    cb = lin_regress_matrix(model_ds[metric_varls[1]], model_ds[metric_varls[0]])            
    qplt.plot(cb, color='black', linestyle='solid', label=ds_labels[1])

    # obs datasets can have different time range..
    obs1, obs2 = obs_extract_overlap(obs_ds[metric_varls[2]], obs_ds[metric_varls[0]])
    ...
    cnts, neg, pos = feedback_nonlin(model_ds[metric_varls[0]], model_ds[metric_varls[1]])
...
def plot_level_4(obs_ds, model_ds, metric_varls, ds_labels):
    ...
    modells = feedback_nonlin(model_ds[metric_varls[0]], model_ds[metric_varls[1]], level4=True)
    ...
    cbar.set_label(f'regression ((W/m2/°C)') #
...
def main(cfg):
    ...
    obs_ds['nhf_eqp_obs'] = rolling_window_statistics(obs_ds['nhf_eqp_obs'], # check index
    ...
    for dataset, mod_ds in model_ds.items():
        ...
        model['nhf_eqp_mod'] = rolling_window_statistics(model['nhf_eqp_mod'], 
                                     coordinate='longitude',operator='mean',window_length=30)

        ds_labels = [f"{obs[0]['dataset']}_{obs[1]['dataset']}", mod_ds[0]['dataset']]
        title = f"net heat flux feedback"
        # plot level 3
        prov_record = get_provenance_record(f'ENSO metrics {metric} feedback level 3', dt_files)
